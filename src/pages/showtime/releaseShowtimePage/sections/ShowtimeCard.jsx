...
const ShowtimeCard = ({ showtime, onRelease, isReleasing }) => {
    return (
        <div className="p-4 transition duration-200 border border-gray-200 rounded-lg hover:border-blue-500">
            <div className="flex items-start gap-4">
                ...
                <div className="flex-shrink-0">
                    <button
                        onClick={() => onRelease(showtime.showtimeId)}
                        disabled={isReleasing}
                        className="px-4 py-2 text-white transition duration-200 bg-green-500 rounded-md hover:bg-green-600 disabled:bg-gray-400 disabled:cursor-not-allowed whitespace-nowrap"
                    >
                        {isReleasing ? 'Đang xử lý...' : 'Công Bố Suất Chiếu'}
                    </button>
                </div>
            </div>
        </div>
